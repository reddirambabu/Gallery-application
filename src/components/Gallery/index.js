...
class Gallery extends Component {
  ...
  onChangeImage = (imageUrl, imageAltText, id) => {
    this.setState({image: imageUrl, altText: imageAltText, activeId: id})
  }

  render() {
    const {image, altText, activeId} = this.state
    ...
    return (
      <div className="app-container">
        <div className="card-container">
          ...
          <p className="image-paragraph">Nature Photography by Rahul</p>
        </div>
        <ul className="thumbnail-container">
          {imagesList.map(eachImageObject => (
            <ThumbnailItem
              ...
              isTrue={eachImageObject.id === activeId}
            />
          ))}
        </ul>
      </div>
